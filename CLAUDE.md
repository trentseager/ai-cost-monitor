...
- `db.py` — raw SQLite (`usage.db`, no ORM) via a `get_conn()` context manager that commits on success. Four tables: `requests` (per-call log), `limits` (per-user daily USD cap), `credit_balances` (per-user prepaid balance), and `reservations` (in-flight holds against a balance, see below). Query helpers (`today_cost_for_user`, `user_summaries_today`, `daily_totals`, `credit_summaries`) return plain dicts/rows for the admin API to serialize directly.
...
4. If the user is credit-metered (`db.has_credit_metering`), size a worst-case reservation from the request (`cfg["estimate_input_tokens"]` × input rate + `max_tokens`-or-fallback × output rate) and atomically reserve it (`db.reserve_credit`). Unknown pricing or insufficient balance → 402, logged the same way as a daily-limit block, **before** contacting the provider. This layers on top of the daily limit, it doesn't replace it. See `docs/credit-reserve-settle.md`.
5. Strip hop-by-hop headers and the inbound `X-User-Id` (`STRIPPED_HEADERS`), forward everything else — including the dev's own `Authorization`/`x-api-key` — verbatim to `cfg["upstream_url"]`.
6. On response, attempt to parse JSON and run `cfg["extract_usage"]`; if usage is present, price it via `pricing.estimate_cost` and log the row via `db.log_request`. If a reservation was made, settle it to the actual cost (`db.settle_reservation`) — refunding the difference between what was reserved and what was actually used. If no usage came back (upstream error/malformed response), release any reservation in full (`db.release_reservation`).
7. Return the upstream response body/status/content-type untouched to the caller, regardless of whether usage parsing succeeded.
