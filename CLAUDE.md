...
- `db.py` — raw SQLite (`usage.db`, no ORM) via a `get_conn()` context manager that commits on success. Two tables: `requests` (per-call log) and `limits` (per-user daily USD cap). Query helpers (`today_cost_for_user`, `user_summaries_today`, `daily_totals`) return plain dicts/rows for the admin API to serialize directly.
...
4. Strip hop-by-hop headers and the inbound `X-User-Id` (`STRIPPED_HEADERS`), forward everything else — including the dev's own `Authorization`/`x-api-key` — verbatim to `cfg["upstream_url"]`.
5. On response, attempt to parse JSON and run `cfg["extract_usage"]`; if usage is present, price it via `pricing.estimate_cost` and log the row via `db.log_request`.
6. Return the upstream response body/status/content-type untouched to the caller, regardless of whether usage parsing succeeded.
...
- **The daily-limit check has a known race** under concurrent requests from the same user (check-then-log isn't atomic). This is accepted for the MVP — don't "fix" it with a lock unless asked, since it changes concurrency behavior.
