...
- On load, fetches `/admin/users` (users overview table, see below), `/admin/daily-totals` (feeds a spend-over-time line chart and a tokens-in/out-over-time bar chart, via Chart.js), `/admin/rate-limits` (see below), and `/admin/credits` (see below).
...
- **Credit balances** ([[credit-reserve-settle]]): a status table (user id, balance, pending reserved, total) plus a top-up form (`user_id` + `amount_usd`, additive only — matches `/admin/credits`' upsert-and-add semantics, no way to set an exact balance or withdraw). `balance_usd` already has any in-flight reservation subtracted, so a caption discloses that `Total = balance + pending` is what full release would restore, to avoid it reading as a discrepancy.
